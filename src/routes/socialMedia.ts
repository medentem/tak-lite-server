...
export function createSocialMediaRouter(
  ...
  const createMonitorSchema = Joi.object({
    ...
    platform: Joi.string().valid('twitter').default('twitter'),
    api_provider: Joi.string().valid('twitterapi_io').default('twitterapi_io'),
    api_credentials: Joi.object({
      api_key: Joi.string().required()
    }).required(),
    search_query: Joi.string().min(1).max(1000).required(),
    query_type: Joi.string().valid('Latest', 'Top').default('Latest'),
  ...
  const updateMonitorSchema = Joi.object({
    ...
    search_query: Joi.string().min(1).max(1000),
    query_type: Joi.string().valid('Latest', 'Top'),
    monitoring_interval: Joi.number().integer().min(60).max(3600),
  ...
  router.get('/monitors', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const safeMonitors = monitors.map(monitor => ({
        ...monitor,
        api_credentials: { api_key: '***' } // Mask the API key
      }));

      res.json({ monitors: safeMonitors });
    } catch (error) {
      next(error);
    }
  });
  ...
  router.get('/monitors/:id', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
  ...
  router.post('/monitors', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.status(201).json({ monitor: safeMonitor });
    } catch (error) {
      next(error);
    }
  });
  ...
  // Update a monitor
  router.put('/monitors/:id', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
      const teamId = (req as any).user.teamId;
      
      const { error, value } = updateMonitorSchema.validate(req.body);
      if (error) {
        return res.status(400).json({ error: error.details[0].message });
      }
      ...
      const updatedMonitor = await socialMediaService.updateMonitor(id, value);
  ...
  // Delete a monitor
  router.delete('/monitors/:id', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
      const teamId = (req as any).user.teamId;
      ...
      await socialMediaService.deleteMonitor(id);
      res.status(204).send();
    } catch (error) {
      next(error);
    }
  ...
  router.post('/test-connection', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { api_key, search_query } = req.body;
      
      if (!api_key || !search_query) {
        return res.status(400).json({ error: 'API key and search query are required' });
      }

      const result = await socialMediaService.testConnection(api_key, search_query);
      res.json(result);
    } catch (error) {
      next(error);
    }
  ...
  router.post('/monitors/:id/start', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
      const teamId = (req as any).user.teamId;
      ...
      await socialMediaService.startMonitoring(id);
  ...
  router.post('/monitors/:id/stop', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
      const teamId = (req as any).user.teamId;
      ...
      await socialMediaService.stopMonitoring(id);
